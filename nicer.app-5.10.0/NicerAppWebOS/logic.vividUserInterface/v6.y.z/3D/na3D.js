...
export class na3D_fileBrowser {
        ...
        */
        ...
        setTimeout(function() {
            ...
            na.m.waitForCondition("animate?", function() {
                //debugger;
                return t.items.length > 250;// && t.winners;
            }, function() {
                //debugger;
                t.create(t);
    ...
    create(t) {
        ...
        na.m.waitForCondition (fncn + ' : t.items ready?', function() {
            return t.items.length > 1000 && t.started4;
        }, function () {
            ...
            for (var i=0; i<1/*intersects.length <-- this just gets an endless series of hits from camera into the furthest reaches of what"s visible behind the mouse pointer */; i++) {
                ...
                while (hoveredItem && !done) {
                    ...
                    if (hoveredItem && hoveredItem.it && !done) {
                        ...
                        t.hoverOverName = hoveredItem.it.name;// + ' ('+intersects.length+')';
                    //debugger;
                        var
                        it = hoveredItem.it,
   ...
   onresize (t, levels) {
    ...
    onresize_do_phase2(t, callback) {
        ...
        for (var i=0; i<t.items.length; i++) {
            ...
            rndMax = (it.ld3 ? (it.ld3.rowColumnCount * 500) : 0);
            rndMax = (p && p.ld3 ? (p.ld3.cubeSideLengthCount * 500) : 0);
                ...
                if (it.name.match(/\.mp3$/)) {
                    if (!t.showFiles) { /*delete it.model;*/} else {
                        it.model.position.x = Math.round( (
                            //(p.model.position.x ? p.model.position.x : 0)
                            //+ (p.ld3?p.column * p.ld3.cubeSideLengthCount * 750:0)
                            (p3.x * it.level)
                            //+ (p?p.p3.x:0)
                            //+((p.column-1) * (p.pos.xField * 750))
                            +((it.column-1)*500)
                        ) / divider);
                        it.model.position.y = Math.round( (
                            //(p.model.position.y ? p.model.position.y : 0)
                            //+ (p.ld3?p.row * p.ld3.cubeSideLengthCount * 750:0)
                            (p?
                                (p3.y * it.level * 1000)
                        //+(p.p3.y)
                                +(-1 * (p.row-1) * (p.pos.xField * 500))
                            : 0)
                            + (p?p.p3.x:0)
                            //+((p.row-1) * p.pos.yField * 500)
                            +((it.row-1)*500)
                            //+ (p.ld3 && it.ld3 ? (p.row * 500) : 0)
                        ) / divider);
                        it.model.position.z = Math.round( (
                            //(p.model.position.z ? p.model.position.z : 0)
                            (p3.x * it.level)
                            + (p?p.p3.x:0)
                            + (p?p.depth*p.level*p.levelIdx*-500:0)
                            //+ (p?p.levelIdx* -3000:0)
                            + (it.level > min ? -1 * p.depth* it.depth * 500 : 0)
                        ) / divider);
                    }
                    ...
                    if (
                        it.name.match(/DJ Firesnake/)
                        || it.name.match(/delinquentes/i)
                        || it.name.match(/Deep House/)
                    ) {};//debugger;

                    it.model.position.y = Math.round( (

                        //(p.model.position.y ? p.model.position.y : 0)
                        //+ (p.ld3?p.pos.yField * p.ld3.cubeSideLengthCount * 500:0)


                        //+ (it.ld3?it.pos.yField * it.ld3.cubeSideLengthCount * 500:0)
                            //+ ((it.row-1) * it.pos.yField * 500)
                        (p3.y * it.level)
                        + (p?p.p3.y:0)
                        +((it.row-1) * 500)
                        //+ (it.level > min ? rndy : 0)
                    ...
                    it.model.position.z = it.depth * 500;
                }
            }
