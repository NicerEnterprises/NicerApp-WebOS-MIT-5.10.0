 ...
 */
    ...
    onload  (forDialogID) {
        ...
        /*
        //setTimeout(function() {
                    delete na.c.menus['#siteToolbarThemeEditor__selector'];
                    na.c.menus['#siteToolbarThemeEditor__selector'] =
                        new naVividMenu ($('#siteToolbarThemeEditor__selector')[0], true);
        //}, 1000);
        */
        delete na.site.c.menus['#textFontFamily'];
        ...
        na.desktop.registerProgress ('[ThemeEditor]', function() {
            ...
            t = na.c.menus['#siteToolbarThemeEditor__selector'];
            if (t) {
                var itEl2 = t.items[0].b.el;
                //debugger;
                t.showMenuItem (
    ...
    initSelectorsTree  (dat) {
        ...
        $('#themeEditor_jsTree_selectors').css({
            ...
            types : {
                'naSelectorSet' : {
                    'icon' : '/siteMedia/na.view.tree.selectorSet.png',
                    'valid_children' : [ 'naElement', 'naSelectorSet', 'naCSS' ]
                },
                'naCSS' : {
                    'icon' : '/siteMedia/na.view.tree.css.png',
                    'valid_children' : [ 'naElement' ]
                },
                "naElement" : {
                    'icon' : '/siteMedia/na.view.tree.element.png',
                    'valid_children' : []
                }
            },
    ...
    transform_jsTree_to_siteGlobalsThemes__do  (it, themeSettings) {
        ...
        /*
         * na.loadTheme(), .saveTheme() and .fetchTheme() distribute and collect the settings under the
         ...
         * to be used in na.saveTheme() and na.loadTheme()
         */
            ...
            case 'naElement' :
                ...
                if (
                    ...
                    themeSettings[parent.text].css = $.extend (
                        themeSettings[parent.text].css, na.fetchTheme(it.text)
                    );
                }
                break;
        }
    ...
    specificitySelected  (event) {
        ...
        if (idx) {
           ...
           na.site.globals.themeName = s.themeName;
            if (!s || (!s.role && !s.user)) {
                na.c.buttons['#btnDeleteSpecificity'].disable();
            } else {
                na.c.buttons['#btnDeleteSpecificity'].enable();
            }

            debugger;
            na.loadTheme (function () { // **POSSIBLY** NOT NEEDED
    ...
    makeThemesList  (themeName) {
        ...
        ac = {
            ...
            success  (data, ts, xhr) {
                ...
                na.setSiteLoginLogout();
                //setTimeout (na.setSpecificity,250);
            },
            error  (xhr, textStatus, errorThrown) {
                na.ajaxFail(fncn, url, xhr, textStatus, errorThrown);
            }                
    ...
    deleteTheme  (event) {
        ...
        ajaxCmd = {
            ...
            success  (data, textStatus, xhr) {
                ...
                } else na.fail(fncn+' : '+url+' : '+data, xhr, textStatus, null);
            },
            error  (xhr, textStatus, errorThrown) {
                na.ajaxFail(fncn, url, xhr, textStatus, errorThrown);
            }                
    ...
    onchange_themeName  (event) {
        ...
        na.te.deleteTheme(false, function() {
            na.saveTheme (function(themeData,data) {
    ...
    borderSettingsSelected  (color) {
        ...
        /*if (na.te.s.c.fireSaveTheme) */na.saveTheme();
    }
...
debugger;
        ...
        $('.themeEditorComponent, .themeEditorComponent_containerDiv2').not('#textSettings').fadeOut('fast', function () {
            if ($('#textSettings').css('display')==='none')
                $('#textSettings').fadeIn('fast', 'swing', function () {
                    ...
                    /*
                    setTimeout(function() {
                        if (!na.c.menus['#textFontFamily'])
                        na.c.menus['#textFontFamily'] = new naVividMenu($('#textFontFamily')[0], true, function(menu) {
                        });
                    }, 500);
                    */
    ...
    textBackgroundOpacityChange  (evt) {
        var 
        fncn = 'na.te.textBackgroundOpacityChange(evt)->na.saveTheme(*callback*)',
        ...
        na.saveTheme(function() {
            /*
            na.loadTheme (function () { // also calls ajax_get_pageSpecificSettings.php
                var
                btn = $('#'+na.te.s.c.selectedButtonID)[0],
                evt = { currentTarget : $('#specificity')[0] };
